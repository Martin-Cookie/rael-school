...
- Auth role helpers: `isAdmin()`, `isManager()`, `canEdit()`, `isSponsor()` z `@/lib/auth`
...
- **CSRF double-submit cookie** — `src/middleware.ts` + `src/lib/csrf.ts` + `src/lib/fetchWithCsrf.ts`; frontend používá `fetchWithCsrf()` místo `fetch()` pro POST/PUT/DELETE
- **Audit trail** — `AuditLog` Prisma model + `logAudit()` z `@/lib/auditLog` (nikdy neshodí request); admin UI v `src/components/admin/AuditLogSection.tsx`
- **CSP + HSTS** hlavičky v `next.config.js` (Content-Security-Policy, Strict-Transport-Security)
- **Rate limiting** na CRUD endpointech: `checkRateLimit()` z `@/lib/rateLimit` (payments, students, approve)
- **Magic bytes validace** na upload endpointech (profile-photo, photos) — ověření JPEG/PNG/GIF/WebP hlaviček
- **PRAGMA integrity_check** po obnově databáze ze zálohy
...
| `useFetchList(url, key)` | `src/hooks/useFetchList.ts` | Generický hook pro fetch seznamu z API — vrací `[data, fetchFn]` |
...
| Utility | Soubor | Popis |
|---|---|---|
| `createCodelistHandlers()` | `src/lib/codelistRoute.ts` | Generická factory pro CRUD codelist routes — používá 6 admin API routes |
| `API_LIMITS` | `src/lib/constants.ts` | Konstanty pro API limity: `{ PAYMENTS: 1000, DASHBOARD_RECENT: 10 }` |
| `studentSchema` | `src/lib/validations.ts` | Zod validační schéma pro studenta |
| `logAudit()` | `src/lib/auditLog.ts` | Audit log — záznam akcí (CREATE, UPDATE, DELETE, LOGIN, APPROVE, REJECT, SPLIT) |
| `fetchWithCsrf()` | `src/lib/fetchWithCsrf.ts` | Fetch wrapper přidávající CSRF token do mutujících requestů |
| `IMAGE_MAX_WIDTH`, `IMAGE_QUALITY` | `src/lib/imageUtils.ts` | Konstanty pro kompresi obrázků (1600px, 0.8 kvalita) |
...
**Stránkování** — platby používají cursor-based stránkování (`Načíst další` tlačítko), ostatní seznamy zobrazují vše najednou
...
| Sekce | Sloupce (šířky) |
|-------|----------------|
| Vybavení | checkbox 4%, typ 22%, stav 11%, poznámky 63% |
...
- Sticky hlavičky: `bg-[#fafaf8] dark:bg-gray-900` (stránky), `bg-gray-50 dark:bg-gray-800` (thead)
- Tabulkové řádky: `border-gray-50 dark:border-gray-700`
- Tabulkové řádky hover: `hover:bg-gray-50 dark:hover:bg-gray-700/50`

### Dialog a11y vzory

- Modální dialogy: `role="dialog"` + `aria-labelledby="dialog-title-id"`
- Nadpis dialogu: `<h3 id="dialog-title-id">`
- Focus trap: `const ref = useFocusTrap<HTMLDivElement>(isOpen)` + `ref={ref}` na kontejner
- Focus-visible na interaktivních prvcích: `focus-visible:ring-2 focus-visible:ring-primary-500 dark:focus-visible:ring-primary-400 focus-visible:outline-none`
