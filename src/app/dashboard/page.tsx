  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-4">
        {statCards.map(card => (
          <button key={card.key} onClick={() => { setActiveTab(card.key); prevTabRef.current = null; setSortCol(''); setSelectedClass(null); setCurrentPage(1) }} className={`bg-white rounded-xl border-2 p-5 card-hover text-left transition-all ${activeTab === card.key ? card.borderColor : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-100">
            ...
            {sortData(students, sortCol).slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE).map((s: any) => (
              <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="py-3 px-3 text-sm">{s.className ? <button onClick={() => { prevTabRef.current = activeTab; setActiveTab('classes'); setSelectedClass(s.className); setSortCol(''); setCurrentPage(1) }} className="text-primary-600 hover:underline">{s.className}</button> : '-'}</td>
