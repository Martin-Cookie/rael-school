...
import { Users, CreditCard, HandHeart, AlertCircle, ChevronUp, ChevronDown, ArrowUpDown, GraduationCap } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch('/api/dashboard').then(r => r.json()).then(data => {
      setStats(data.stats); setRecentPayments(data.recentPayments || []); setStudents(data.students || []); setSponsors(data.sponsors || []); setStudentsWithNeeds(data.studentsWithNeeds || []); setLoading(false)
    }).catch(() => setLoading(false))
  }, [])

  function handleSort(col: string) { if (sortCol === col) setSortDir(sortDir === 'asc' ? 'desc' : 'asc'); else { setSortCol(col); setSortDir('asc') } }

  function sortData<T>(data: T[], col: string): T[] {
    if (!col) return data
    return [...data].sort((a: any, b: any) => {
      let va = col.startsWith('_count.') ? a._count?.[col.replace('_count.','')] ?? 0 : col.includes('.') ? col.split('.').reduce((o,k) => o?.[k], a) : a[col]
      let vb = col.startsWith('_count.') ? b._count?.[col.replace('_count.','')] ?? 0 : col.includes('.') ? col.split('.').reduce((o,k) => o?.[k], b) : b[col]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        {statCards.map(card => (
          <button key={card.key} onClick={() => { setActiveTab(card.key); setSortCol(''); setSelectedClass(null) }} className={`bg-white rounded-xl border-2 p-5 card-hover text-left transition-all ${activeTab === card.key ? card.borderColor : 'border-gray-200 hover:border-gray-300'}`}>
            <div className="flex items-start justify-between"><div><p className="text-sm text-gray-500 mb-1">{card.label}</p><p className="text-2xl font-bold text-gray-900">{card.value}</p></div><div className={`w-10 h-10 rounded-xl flex items-center justify-center ${card.color}`}><card.icon className="w-5 h-5" /></div></div>
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        {/* Payments */}
        {activeTab === 'payments' && (
          <div><h2 className="text-lg font-semibold text-gray-900 mb-4">{t('payments.title')} ({recentPayments.length})</h2>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-100">
              ...
              <SH col="source" className="text-left">{t('payments.source')}</SH>
            </tr></thead><tbody>
              {sortData(recentPayments, sortCol).map((p: any) => (
