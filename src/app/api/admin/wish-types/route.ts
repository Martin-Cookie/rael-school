...
export async function POST(request: NextRequest) {
  try {
    ...
    const wishType = await prisma.wishType.create({ data: { name: name.trim(), sortOrder: sortOrder ?? 0, price: price ?? null } })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.orders && Array.isArray(body.orders)) {
      ...
      await prisma.wishType.update({ where: { id: body.id }, data: { price: body.price ?? null } })
    } else {
