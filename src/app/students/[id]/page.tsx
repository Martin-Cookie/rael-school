...
export default function StudentDetailPage({ params }: { params: { id: string } }) {
  ...
  const [newVoucher, setNewVoucher] = useState({ type: 'purchase', date: '', amount: '', count: '', notes: '' })
  ...
  useEffect(() => {
    ...
    const savedCurrency = localStorage.getItem('rael-currency')
  ...
  async function handleSave() {
    ...
    try {
      ...
      await fetch(`/api/students/${id}/equipment`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ equipment: editEquipment }),
      })
      if (res.ok) {
        await fetchStudent()
        setEditMode(false)
  ...
  function cancelEdit() {
    setEditData(student)
    setEditEquipment(student.equipment?.map((eq: any) => ({ ...eq })) || [])
    setEditMode(false)
  }

  function changeCurrency(c: string) {
    setCurrency(c)
    localStorage.setItem('rael-currency', c)
  }
  ...
  async function addNeed() {
    ...
    try {
      const res = await fetch(`/api/students/${id}/needs`, {
  ...
  async function toggleNeedFulfilled(needId: string, current: boolean) {
    try {
      ...
      await fetch(`/api/students/${id}/needs`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ needId, description: need.description, isFulfilled: !current }),
      })
      await fetchStudent()
    } catch { showMsg('error', t('app.error')) }
  }
  ...
  async function addVoucher() {
    ...
    try {
      const res = await fetch(`/api/students/${id}/vouchers`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newVoucher),
      })
      if (res.ok) { setNewVoucher({ type: 'purchase', date: '', amount: '', count: '', notes: '' }); setShowAddVoucher(false); await fetchStudent(); showMsg('success', t('app.savedSuccess')) }
    } catch { showMsg('error', t('app.error')) }
  }
  ...
  async function addHealthCheck() {
    ...
    try {
      const res = await fetch(`/api/students/${id}/health`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteHealthCheck(checkId: string) {
    if (!confirm(t('app.confirmDelete'))) return
    try {
      await fetch(`/api/students/${id}/health`, {
        method: 'DELETE', headers: { 'Content-Type': 'application/json' },
  ...
  async function addPhoto() {
    ...
    try {
      ...
      if (newPhoto.takenAt) formData.append('takenAt', newPhoto.takenAt)
      const res = await fetch(`/api/students/${id}/photos`, { method: 'POST', body: formData })
  ...
  async function addSponsor() {
    if (!newSponsor.firstName || !newSponsor.lastName || !newSponsor.email) {
      showMsg('error', 'Vyplňte jméno, příjmení a email / Fill in name and email')
      return
    }
    try {
      ...
      if (res.ok) {
        setNewSponsor({ firstName: '', lastName: '', email: '', phone: '', startDate: '', notes: '' })
        setShowAddSponsor(false); await fetchStudent(); showMsg('success', t('app.savedSuccess'))
      } else {
        const data = await res.json(); showMsg('error', data.error || t('app.error'))
      }
    } catch { showMsg('error', t('app.error')) }
  }
  ...
  async function saveSponsorEdit(sponsorshipId: string, sponsorUserId: string) {
    try {
      await fetch(`/api/students/${id}/sponsors`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sponsorshipId, sponsorUserId, ...editSponsorData }),
      })
  ...
  function ensureEquipmentItems() {
    ...
    const current = editEquipment.map((eq: any) => eq.type)
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 overflow-x-auto border-b border-gray-200 pb-px">
        {tabs.map((tab) => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors border-b-2 -mb-px ${activeTab === tab.key ? 'border-primary-600 text-primary-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                {student.needs?.map((need: any) => (
                  <div key={need.id} className={`flex items-center justify-between p-3 rounded-lg ${need.isFulfilled ? 'bg-primary-50' : 'bg-red-50'}`}>
                    <div className="flex items-center gap-3">
                      {canEditData ? (
                        <button onClick={() => toggleNeedFulfilled(need.id, need.isFulfilled)} className={`w-6 h-6 rounded-full flex items-center justify-center border-2 transition-colors ${need.isFulfilled ? 'bg-primary-500 border-primary-500' : 'border-gray-300 hover:border-primary-400'}`}>{need.isFulfilled && <Check className="w-4 h-4 text-white" />}</button>
                      ) : (
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center ${need.isFulfilled ? 'bg-primary-500' : 'bg-gray-300'}`}>{need.isFulfilled && <Check className="w-4 h-4 text-white" />}</div>
                      )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              ...
              <div className="flex items-center gap-2">
                <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
                  {[{ key: 'all', label: t('photos.filterAll') }, { key: 'visit', label: t('photos.visit') }, { key: 'handover', label: t('photos.handover') }, { key: 'voucher', label: t('photos.voucher') }].map((f) => (
                    <button key={f.key} onClick={() => setPhotoFilter(f.key)} className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${photoFilter === f.key ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{f.label}</button>
                  ))}
                </div>
            ...
            {showAddPhoto && (
              <div className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                  <select value={newPhoto.category} onChange={(e) => setNewPhoto({ ...newPhoto, category: e.target.value })} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none">
                    <option value="visit">{t('photos.visit')}</option><option value="handover">{t('photos.handover')}</option><option value="voucher">{t('photos.voucher')}</option>
                  </select>
                  <input type="date" value={newPhoto.takenAt} onChange={(e) => setNewPhoto({ ...newPhoto, takenAt: e.target.value })} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" placeholder={t('photos.takenAt')} />
                  <input type="text" value={newPhoto.description} onChange={(e) => setNewPhoto({ ...newPhoto, description: e.target.value })} placeholder={t('photos.description')} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                ...
                <div className="flex gap-2">
                  <button onClick={addPhoto} disabled={!newPhoto.file} className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 disabled:opacity-50">{t('photos.upload')}</button>
                  <button onClick={() => setShowAddPhoto(false)} className="px-3 py-2 text-gray-500 hover:text-gray-700 text-sm">{t('app.cancel')}</button>
                </div>
              </div>
            )}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <select value={currency} onChange={(e) => changeCurrency(e.target.value)} className="px-2 py-1 rounded-lg border border-gray-300 text-xs font-medium focus:ring-2 focus:ring-primary-500 outline-none">
                  {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              {canEditData && <button onClick={() => setShowAddVoucher(true)} className="flex items-center gap-1.5 text-sm text-primary-600 hover:text-primary-700 font-medium"><Plus className="w-4 h-4" /> {t('app.add')}</button>}
            </div>
            {showAddVoucher && (
            ...
            <div className="overflow-x-auto mb-6">
              <table className="w-full table-fixed"><thead><tr className="border-b border-gray-200">
                ...
                {(!student.vouchers || student.vouchers.length === 0) && <tr><td colSpan={4} className="py-4 text-center text-gray-500 text-sm">{t('app.noData')}</td></tr>}
              </tbody></table>
            </div>
            {/* Usages table */}
        ...
        {activeTab === 'sponsors' && (
          <div>
            ...
            {showAddSponsor && (
              <div className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                <p className="text-xs text-gray-500 mb-3">Vyplňte údaje sponzora. Pokud sponzor v systému neexistuje, bude vytvořen automaticky.<br/>Fill in sponsor details. If sponsor doesn't exist, they will be created.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  <input type="text" value={newSponsor.firstName} onChange={(e) => setNewSponsor({ ...newSponsor, firstName: e.target.value })} placeholder={t('student.firstName') + ' *'} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <input type="text" value={newSponsor.lastName} onChange={(e) => setNewSponsor({ ...newSponsor, lastName: e.target.value })} placeholder={t('student.lastName') + ' *'} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <input type="email" value={newSponsor.email} onChange={(e) => setNewSponsor({ ...newSponsor, email: e.target.value })} placeholder={t('sponsors.email') + ' *'} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <input type="text" value={newSponsor.phone} onChange={(e) => setNewSponsor({ ...newSponsor, phone: e.target.value })} placeholder={t('sponsors.phone')} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <input type="date" value={newSponsor.startDate} onChange={(e) => setNewSponsor({ ...newSponsor, startDate: e.target.value })} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <input type="text" value={newSponsor.notes} onChange={(e) => setNewSponsor({ ...newSponsor, notes: e.target.value })} placeholder={t('sponsors.notes')} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                </div>
                <div className="flex gap-2">
                  <button onClick={addSponsor} className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">{t('app.add')}</button>
                  <button onClick={() => setShowAddSponsor(false)} className="px-3 py-2 text-gray-500 hover:text-gray-700 text-sm">{t('app.cancel')}</button>
                </div>
              </div>
            )}
                    ...
                    {editingSponsor === sp.id ? (
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <input type="text" value={editSponsorData.notes || ''} onChange={(e) => setEditSponsorData({ ...editSponsorData, notes: e.target.value })} placeholder={t('sponsors.notes')} className="px-3 py-2 rounded-lg border border-gray-300 text-sm" />
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => saveSponsorEdit(sp.id, sp.sponsor.id)} className="px-3 py-1.5 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">{t('app.save')}</button>
                          <button onClick={() => setEditingSponsor(null)} className="px-3 py-1.5 text-gray-500 hover:text-gray-700 text-sm">{t('app.cancel')}</button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-start gap-4">
                        ...
                        {canEditData && (
                          <button onClick={() => { setEditingSponsor(sp.id); setEditSponsorData({ firstName: sp.sponsor.firstName, lastName: sp.sponsor.lastName, phone: sp.sponsor.phone || '', notes: sp.notes || '' }) }} className="p-2 text-gray-400 hover:text-gray-600"><Pencil className="w-4 h-4" /></button>
                        )}
                      </div>
                    )}
                  </div>
            ...
            {showAddHealth && (
              <div className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                  <input type="date" value={newHealth.checkDate} onChange={(e) => setNewHealth({ ...newHealth, checkDate: e.target.value })} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                  <select value={newHealth.checkType} onChange={(e) => setNewHealth({ ...newHealth, checkType: e.target.value })} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none"><option value="general">{t('health.general')}</option><option value="dentist">{t('health.dentist')}</option><option value="urgent">{t('health.urgent')}</option></select>
                  <input type="text" value={newHealth.notes} onChange={(e) => setNewHealth({ ...newHealth, notes: e.target.value })} placeholder={t('health.notes')} className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
                </div>
                <div className="flex gap-2">
                  <button onClick={addHealthCheck} className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">{t('app.add')}</button>
                  <button onClick={() => setShowAddHealth(false)} className="px-3 py-2 text-gray-500 hover:text-gray-700 text-sm">{t('app.cancel')}</button>
                </div>
              </div>
            )}
...
function Field({ label, value, type = 'text', editMode, onChange }: { label: string; value: any; type?: string; editMode: boolean; onChange: (v: string) => void }) {
  return (<div><label className="block text-sm font-medium text-gray-500 mb-1">{label}</label>
    {editMode ? <input type={type} value={value || ''} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 outline-none" /> : <p className="text-gray-900 py-2">{type === 'date' ? formatDate(value) : value || '-'}</p>}
  </div>)
}
...
function SelectField({ label, value, editMode, options, displayValue, onChange }: { label: string; value: string; editMode: boolean; options: { value: string; label: string }[]; displayValue: string; onChange: (v: string) => void }) {
  return (<div><label className="block text-sm font-medium text-gray-500 mb-1">{label}</label>
    {editMode ? <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 outline-none">{options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select> : <p className="text-gray-900 py-2">{displayValue}</p>}
  </div>)
}
