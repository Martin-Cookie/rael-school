    ...
    <div className="bg-white rounded-xl border border-gray-200 card-hover overflow-hidden">
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-3">
          ...
          {showRestoreConfirm && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="restore-dialog-title" ref={restoreModalRef} onKeyDown={(e) => e.key === 'Escape' && setShowRestoreConfirm(false)}>
              <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center">
                    <AlertTriangle className="w-5 h-5 text-amber-600" />
                  </div>
                  <h3 id="restore-dialog-title" className="text-lg font-semibold text-gray-900">{t('admin.restoreBackup')}</h3>
                </div>
